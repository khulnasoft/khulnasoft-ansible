...
checkPackages(){
    ...
    if [ $1 == "production" ]; then
        echo "production"
        PACKAGES_URL=https://packages.khulnasoft.com/4.x/
    elif [ $1 == "pre-release" ]; then
        echo "pre-release"
        PACKAGES_URL=https://packages-dev.khulnasoft.com/pre-release/
    elif [ $1 == "staging" ]; then
        echo "staging"
        PACKAGES_URL=https://packages-dev.khulnasoft.com/staging/
        CHECK_WIN_PACKAGE=$(grep windows ../files/packages_uri_new.txt)
        echo $CHECK_WIN_PACKAGE
        if [ -n "$CHECK_WIN_PACKAGE" ]; then
            WIN_AGENT_NAME=$(aws s3 ls s3://packages-dev.khulnasoft.com/staging/windows/khulnasoft-agent-$VERSION --region=us-west-1 | tail -1 | awk '{printf $4}')
            ...
            sed -i 's,khulnasoft_winagent_config_url.*,khulnasoft_winagent_config_url: \"'$PACKAGES_URL$WIN_AGENT_URI'\",g' ../../vars/repo_staging.yml
            sed -i 's,khulnasoft_winagent_package_name.*,khulnasoft_winagent_package_name: \"'$WIN_AGENT_NAME'\",g' ../../vars/repo_staging.yml
        fi
    fi
